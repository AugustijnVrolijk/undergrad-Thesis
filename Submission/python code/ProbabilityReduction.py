...
class Node():
    def __init__(self, depth, momentum, transitionMatrix, isRoot, best):
        self.branches = []
        self.expired = []
...
class TreeNode(Node):
    def __init__(self, parent, transition, momentum, isRoot) -> None:
        super().__init__(depth = parent.depth + 1, momentum=momentum, transitionMatrix=parent.transitionMatrix,isRoot=isRoot, best=parent.best)
        self.parent = parent
    ...
    def updateSelf(self, score, corrVal, MomentumPen = 0.005):
        if score < self.best:
            self.best = score
            super().updateMomentum(isNewBest=True, MomentumPen=MomentumPen)
        else:
            super().updateMomentum(isNewBest=False, MomentumPen=MomentumPen)
    ...
    def updateTransition(self, score, corrVal, MomentumPen):
        if score < self.best:
            self.best = score
            super().updateMomentum(True, MomentumPen)
        isNewBest = self.transition.updateTransition(score, corrVal, MomentumPen)
        if isNewBest:
            self.transition.updateMomentum(isNewBest, MomentumPen)
        self.parent.updateTransition(score, corrVal, MomentumPen)
...
class Transition(Node):
    def __init__(self, val, dimension, transitionMatrix) -> None:
        super().__init__(depth=1, momentum=1, transitionMatrix=transitionMatrix,isRoot = True, best=np.inf)
...
def main():
    ...
    agent = TransitionMatrixTree(graph, posTransitions, len(posTransitions), 700, 700, 30, 0.8, 0.2, 35, 0.9, 1.1)
    best = agent.mainRun()
    print(best)
